        ...
        if(handles.time_show==true){
            ...
            if (refreshDatePool.call(handles, self, "")) {
                ...
                if($(handles.config.input).length>0){
                    ...
                    if(newDate<today&&handles.config.enabled!='true'){return false;}
                    ...
                    //判断是否自动隐藏
                    var v1=$(handles.config.input).eq(0).val();
                    var v2=$(handles.config.input).eq(1).val();
                    //console.log(v1,v2,handles.config.autohide,$(handles.config.autohidenode).length);
                    if(v1!=""&&v2!=""&&handles.config.autohide=="true"){
                        $(handles.config.autohidenode).hide();
                    }
                    //增加遮罩选取日期
