	...
	function success(){
		...
		*/
		...
		if(newM==2){
            $(".step-bn").addClass('l');
			setTimeout(function(){
				$(".step-bn").addClass('active');
			},1);
			$('.step-list').find("li").each(function(k,d){
				var $d=$(d);
				$d.addClass('l');
				setTimeout(function(){
		...
		function handleToThree(o){
			changeClose();
			var  secOneH=$(".main").find('.m-page').eq(0).outerHeight();
			var  secTwoH=$(".main").find('.m-page').eq(1).outerHeight();
			var  secThreeH=$(".main").find('.m-page').eq(2).outerHeight();
			$('.u-arrow').hide();
			$('.to-top').addClass('to-top-active');
			$(".m-page").removeClass('hide').addClass("relative");
			$(".main").css({
				height:secOneH+secTwoH+secThreeH+"px",
				overflow:'visible'
			});
			if(o==""){
				$(".main").find('.m-page').eq(1).css({
