...
<head>
    ...
    {include "Web_Mobile_Static::space.v2.0/inc.head_space.tpl"}
</head>
...
{*list*}
<div class="list-item list-item-result">
    <ul>
        {foreach $page->getSearchResult as $k=>$v}
        <li>
            <a href="">
                <div class="list-item-left">
                    <img src="{Page_Site_Global::displayMedia($v['logo'],150,113)}">
                </div>
                <div class="list-item-right">
                    <p class="name"><span class="text">{if $v['item_name']==$v['baike_name']}{$v['baike_name']}{else}{$v['baike_name']}[{$v['item_name']}]{/if}</span><span class="count">指数：8</span></p>
                    <p class="address">地址: {$v['addr']}</p>
                    <p class="price">参考价格:&nbsp;
                        <span>{if $v['price']>0}<span1>¥&nbsp;</span1>{$v['min_price']}--{$v['max_price']}/{$v['price_unit']}{else}面议{/if}</span></p>
                </div>
                <div class="clear"></div>
            </a>
        </li>
