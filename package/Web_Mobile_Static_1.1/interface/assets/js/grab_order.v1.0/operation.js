...
operation = {
    grabSingle: function (e) {
        ...
        YunSpace.api('grab/order/grabSingle.json', {
            ...
            alert(data.msg);
            if (data.status == 1) {
                window.location.href = "?type=B&action=detail&id=" + id;
            } else {
    ...
    recordSubmit: function (e) {
        //联系记录提交
        var id = $(e).attr('data-id');
        var _step = $("#col-selected").find("option:selected").val();
        var _clos_reason = $("input[name=reason]:checked").val();

        var _conent = $("#record_content").val();
        if (!_conent) {
            if (_clos_reason == 2000) {
                var _other = $(".reason-textarea textarea").val();
                _conent = '其他原因：' + _other;
            } else {
                _conent = _clos_reason;
            }
        }
        if (_conent.length < 4) {
            alert('原因输入过少！');
        ...
        var tags = $(".activity-needs-list input[name=tags]").val();
