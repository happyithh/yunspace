...
switch (Core::$urls['action']) {
    ...
    case 'forget_step2':
        ...
        switch ($_REQUEST['action']) {
            ...
            case 'code':
                ...
                break;
            case'code_new':
                forgetLogin::identityPhoneNew();
                break;
            case 'next':
                forgetLogin::clickNext();
                break;
            case'next_new':
                forgetLogin::clickNextNew();
                break;
            default:
    ...
    case 'forget_step3':
        ...
        break;
    case 'forget_step3_new':
        forgetLogin::updatePasswordNew();
        break;
}
...
//验证手机号码
    ...
    //验证验证码_New
    static function identityPhoneNew()
    {
        @session_start();
        if ($_REQUEST['code'] == $_SESSION['auth_code'][1]) {
            Func_Output::json('success', '', '成功');
        } else {
            Func_Output::json('error', '', '验证失败');
        }
    }
    ...
    static function clickNextNew()
    {
        ...
        $data = Data_Mysql_Table_Account::select('*', $conditions, '');
        session_start();
        if (empty($_SESSION['auth_code'][0]) || $_REQUEST['phone'] != $_SESSION['auth_code'][0]) {
            Func_Header::back('验证手机不一致！');
        }
        if (empty($_SESSION['auth_code'][1]) || empty($_REQUEST['code']) || $_REQUEST['code'] != $_SESSION['auth_code'][1]) {
            Func_Header::back('验证码错误！');
        }
        if (!$data) {
            Func_Header::back('该手机号码没有绑定！！');
    ...
    static function updatePasswordNew()
    {
        session_start();
        $data = array();
        $data['password'] = Data_Mysql_Table_Account::makePassword($_REQUEST['password']);
        ...
        if ($update) {
            ...
            Data_Mysql_Table_Account_Log::insertAccountLog($rs[0]['id'], 0, '修改密码', '更新--修改密码');
            Func_Header::to('密码修改成功！','/user_center/login');
        }
    }
}