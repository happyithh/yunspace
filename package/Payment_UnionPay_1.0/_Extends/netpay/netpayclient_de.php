...
{
    $hash = sha1($string);
    $sha_bin = hex2bin_($hash);
    $sha_pad = hex2bin_(HASH_PAD);
...
{
    $p = bin2int_($private_key["prime1"]);
    $q = bin2int_($private_key["prime2"]);
    $u = bin2int_($private_key["coefficient"]);
    $dP = bin2int_($private_key["prime_exponent1"]);
    $dQ = bin2int_($private_key["prime_exponent2"]);
    $c = bin2int_($input);
    $cp = bcmod($c, $p);
    $cq = bcmod($c, $q);
...
function rsa_decrypt($input)
{
    ...
    $modulus = bin2int_($private_key["modulus"]);
...
function padstr($str)
{
    return str_pad($str, 256, '0', STR_PAD_LEFT);
}