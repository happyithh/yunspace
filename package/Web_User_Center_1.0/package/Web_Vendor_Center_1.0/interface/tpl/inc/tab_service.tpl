                ...
                <tr data-pid="{$pv['id']}">
                    ...
                    <td style="position: relative!important;">
                        ...
                        {else}
                            <select class="form-control input-sm" onchange="ListOperator.online(this)">
                                <option value="1" {if $pv['status']== '1'} selected {/if}>
                                    上线
                                </option>
                                <option value="0" {if $pv['status']== '0'} selected {/if}>
                                    下线
                                </option>
                            </select>
                        {/if}
                    ...
                    <td>
                        ...
                        {if $pv['update_status']==1 || $pv['update_status']==2}
                            ...
                            {if $pv['update_status'] == 0 || $pv['update_status'] == 1}
                                <a href="{$_route}product_edit/schedule?product_id={$pv['id']}"
                                   class="btn btn-xs btn-info">档期</a>
                            {/if}
                            {if $pv['update_status'] == 2 || $pv['update_status'] == 3}
                                <a href="javascript:;" onclick="alert('请等待审核通过后，您可以添加档期，能更快成单哦！')"
                                   class="btn btn-xs btn-info">档期</a>
                            {/if}
                            {if $pv['update_status'] != 2}
                                <a href="javascript:;" data-status="-1" onclick="ListOperator.handle_product(this)"
                                   class="btn btn-xs btn-danger delpro">删除</a>
                            {/if}
                        {/if}
