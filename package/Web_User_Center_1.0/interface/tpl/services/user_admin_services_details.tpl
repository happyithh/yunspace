...
<div class="portlet">
    <div class="portlet-body util-btn-margin-bottom-5" style="height:90px;border-bottom:1px solid #e4e4e4">
        <form action="?action=detail&demand_id={$_REQUEST['demand_id']}" method="post" claassetss="pull-right">
            <div class="btn-group pull-left" style="margin-right: 10px;">
                <select class="form-control input-small select2me pull-left" name="category">
                    <option {if $_REQUEST['category']==''} selected{/if} value="">全部</option>
                    <option {if $_REQUEST['category']=='space'} selected{/if} value="space">空间场地</option>
                    <option {if $_REQUEST['category']=='service'} selected{/if} value="service">配套服务</option>
                </select>
            </div>
            <div class="input-group input-medium">
                <input type="text" name="product_name" value="{$_REQUEST['product_name']}" class="form-control" placeholder="请输入产品名称">
    ...
    <div class="table-responsive" style="border:0px;">
        <table class="table table-hover" style="border:0px;">
            ...
            {if $data['demand_recommend_demo']}
                ...
                {/foreach}
            {/if}
            {if $data['demand_recommend_product']}
                <tr  class="portlet-title">
                    <td colspan="3">
                        <h5><strong>推荐产品列表</strong></h5>
                    </td>
                </tr>
                {foreach $data['demand_recommend_product']['rows'] as $k=>$v}
                <tr>
                    <td width="270px">
                        <div>
                            <a href="{$_root}service_info/{$v['id']}.html" target="_blank">
                                {if $v['logo']}
                    ...
                    <td width="300px">
                        ...
                        <div class="pull-right" style="text-align: center;  width: 232px;margin-top: 20px;">
                            <a target="_blank" class="btn btn-info" href="?action=pdf&product_id={$v['id']}">导出PDF</a>
                        </div>
                    </td>
                </tr>
            {/foreach}
