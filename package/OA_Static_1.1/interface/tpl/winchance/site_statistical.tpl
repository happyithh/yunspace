                        ...
                        <div class="form-group col-md-5">
                            ...
                            <div class="col-md-4">
                                <input type='text' class="form-control date" placeholder="请选择开始时间"
                                       name="search[start_time]" value="{if !empty($_REQUEST['search']['start_time'])}{$_REQUEST['search']['start_time']}{else}{date('Y-m-01')}{/if}"/>
                            </div>
                            <div class="col-md-4">
                                <input type='text' class="form-control date" placeholder="请选择结束时间"
                                       name="search[end_time]" value="{if !empty($_REQUEST['search']['end_time'])}{$_REQUEST['search']['end_time']}{else}{date('Y-m-d')}{/if}"/>
                            </div>

                        </div>
                        {*<div class="form-group col-md-5">*}
                            {*<label class="col-md-3 control-label">进度类型</label>*}
            ...
            <div class="tab-content clearfix">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered ">
                        ...
                        {foreach $data as $k=>$v}
                            <tr class="mHidden">
                                <td>{if $k!=''}{$step[$k]}{else}暂无进度{/if}</td>
                                {foreach $v as $kk=>$vv}
                                    <td>{$vv}</td>
                                {/foreach}
                                {if !empty($v)}
