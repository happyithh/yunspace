                        ...
                        <div class="form-group col-md-5">
                            ...
                            <div class="col-md-9">
                                <select class="form-control input-sm" name="search[demand_level]">
                                    <option value='' {if !isset($_REQUEST['search']['demand_level'])}selected {/if}>全部</option>
                                    <option value=2 {if $_REQUEST['search']['demand_level']==2}selected {/if}>优质</option>
                                    <option value=1 {if $_REQUEST['search']['demand_level']==1}selected {/if}>中等</option>
                                    <option value=0 {if isset($_REQUEST['search']['demand_level']) && $_REQUEST['search']['demand_level']==0}selected {/if}>普通</option>
                                </select>
                            </div>
                        </div>
                    </div>
    ...
    <div class="portlet-body">
        <ul class="nav nav-tabs">
            ...
            <li class="{if $type=='waiting'}active{/if}">
                <a href="?action=list&type=waiting{if !empty($_REQUEST['search'])}&{$urls}{/if}">待处理（{($_waitingtotal)}
                    ）</a>
            </li>
            ...
            <li class="{if $type=='abandon'}active{/if}">
                <a href="?action=list&type=abandon{if !empty($_REQUEST['search'])}&{$urls}{/if}">已放弃（{($_abandontotal)}
                    ）</a>
            </li>
            <li class="{if $type=='offer'}active{/if}">
                <a href="?action=list&type=offer{if !empty($_REQUEST['search'])}&{$urls}{/if}">测试（{($_offertotal)}
                    ）</a>
            </li>
        </ul>
        <div class="tab-content clearfix">
            {include 'OA_Static::demands/operate/operate_demand_pending.tpl'}
        </div>
    </div>
</div>
