                ...
                <form action="?" method="get" class="clearfix">
                    <div class="form-body clearfix">
                        <div class="form-group col-md-5">
                            ...
                            <div style="width: 120px;float: left;margin: 0 10px 0 10px;">
                                <select class="form-control input-sm" name="search[type]">
                                    <option value="1" {if $_REQUEST['search']['type']==1} selected {/if} >产品名称</option>
                                    <option value="2" {if $_REQUEST['search']['type']==2} selected {/if}>套餐名称</option>
                                </select>
                            </div>
                            <div class="col-md-9" style="float: right">
                                <input type="text" class="form-control input-sm"placeholder="ID / 名称" name="search[product_name]" value="{$_REQUEST['search']['product_name']}">
                            </div>
                        </div>

                        <div class="form-group col-md-5">
                            <label class="col-md-3 control-label">销售人员</label>

                            <div class="col-md-9">
                                <input type="text" class="form-control input-sm" placeholder="销售人员名称" name="search[username]" value="{$_REQUEST['search']['username']}">
                            </div>
                        </div>
                        <div class="form-group col-md-5">
        ...
        <div class="table-responsive clearfix">
            <table class="table table-striped table-bordered table-hover" id="data_table" style="word-break: break-all">
                ...
                {foreach $_data['rows'] as $k=>$v}
                    <tr>
                        <td>{if !empty($v['product_name'])}
                                <a href="?search[product_name]={urlencode($v['product_name'])}">
                                {$v['product_name']}</a>{else}<a href="?search[set_name]={$v['set_name']}">{$v['set_name']}</a>{/if}</td>
                        <td>{date('Y-m-d',$v['look_time'])}</td>
                        <td class="list_none">{$v['look_reason']|default:'-----'}</td>
                        <td>{$v['city']}</td>
                        <td><a href="?search[phone]={$v['phone']}">{$v['demand']['联系人']|default:'-----'}</a></td>
                        <td><a href="?search[username]={$v['username']}">{$v['username']}</a></td>
