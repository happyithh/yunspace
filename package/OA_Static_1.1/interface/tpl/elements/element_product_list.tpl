                        ...
                        <div class="form-group col-md-5">
                            ...
                            <div class="col-md-9">
                                {*<select class="form-control input-sm" name="search[search_city]">*}
                                    ...
                                    {*{/foreach}*}
                                {*</select>*}
                                <input type="text" class="form-control input-sm" placeholder="请输入要查询的城市" name="search[search_city]" value="{$_REQUEST['search']['search_city']}">
                            </div>
                        </div>
                        <div class="form-group col-md-5">
...
<div class="portlet">
    ...
    <div class="portlet-tabs" style="position:relative">
        <div style="position: absolute;left:10px;bottom:0px;color: #444444;font-size: 16px">{if $_data['other']['handle_type']=='waiting'}<span class="required" aria-required="true" style="color: red">*</span>此列表不包含首次审核中的产品
            {elseif $_data['other']['handle_type']=='reject'}<span class="required" aria-required="true" style="color: red">*</span>此列表不包含首次审核拒绝的产品{/if}</div>
        <ul class="nav nav-tabs" style="overflow: inherit; padding-left: 100px">

            <li class="{if $_data['other']['handle_type']=='reject'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}handle_type=reject&{$_data['other']['urls']}{else}list_type=product&handle_type=reject{/if}">审核拒绝</a>
            </li>
            <li class="{if $_data['other']['handle_type']=='waiting'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}handle_type=waiting&{$_data['other']['urls']}{else}list_type=product&handle_type=waiting{/if}">审核中</a>
            </li>
            <li class="{if $_data['other']['handle_type']=='offline'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}handle_type=offline&{$_data['other']['urls']}{else}list_type=product&handle_type=offline{/if}">下线</a>
            </li>
            ...
            <table class="table table-striped table-bordered table-hover" id="data_table" style="word-break: break-all">
                ...
                {foreach $_data['rows'] as $k=>$v}
                    <tr>
                        ...
                        <td class="list_none">{$v['attr_index'][46]}</td>
                        <td class="list_size">{$v['order_product_size']}</td>
                        <td class="list_none">{if $v['update_time']>1430713953}{date('Y-m-d H:i:s',$v['update_time'])}{else}--{/if}</td>
                        <td >
                            <a href="?action=detail&id={$v['id']}&list_type=product" class="btn btn-success btn-xs list_size"><i class="fa fa-eye"></i>详情</a>
                            &nbsp;
                            {if $v['status']==1}
                                <a href="http://{Core::$urls['host']}/service_info/{$v['id']}.html" class="btn btn-primary btn-xs list_size" target="_blank"><i class="fa fa-link"></i>链接</a>
                            {/if}
                            &nbsp;
                            {if $_data['other']['is_admin']}
                                <a href="?action=edit&id={$v['id']}&list_type=product" class="btn btn-info btn-xs list_none"><i class="fa fa-pencil"></i>修改</a>
                                &nbsp;
                                {if $v['update_status']!=-2}
                                <a data-pid="{$v['id']}" href="javascript:;" class="btn btn-danger btn-xs deleteProduct list_none"><i class="fa fa-pencil"></i>删除</a>
                                {/if}
                            {/if}
                        </td>
                    </tr>
