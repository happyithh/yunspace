                ...
                <form action="?" method="get" class="clearfix">
                    ...
                    <div class="form-body clearfix">
                        ...
                        <div class="form-group col-md-5">
                            ...
                            <div class="col-md-9">
                                <input type="text" class="form-control input-sm" placeholder="供应商名称"
...
<div class="portlet">
    ...
    <div class="portlet-tabs" style="position:relative">
        <div style="position: absolute;left:10px;bottom:0px;color: #444444;font-size: 16px">
        ...
        <div style="float:right;margin-top:10px;margin-right:30px;color: #444444;font-size: 16px">
            <a href="?{if !empty($_REQUEST['search'])}search[level]=100&{$_data['other']['urls']}{else}search[level]=100{/if}{if isset($_REQUEST['handle_type'])}&handle_type={$_REQUEST['handle_type']}{/if}" class="btn {if !isset($_REQUEST['search']['level']) || $_REQUEST['search']['level']==100}btn-success{/if} btn-default">全部</a>
            <a href="?{if !empty($_REQUEST['search'])}search[level]=4&{$_data['other']['urls']}{else}search[level]=4{/if}{if isset($_REQUEST['handle_type'])}&handle_type={$_REQUEST['handle_type']}{/if}" class="btn {if $_REQUEST['search']['level']==4}btn-success{/if} btn-default">A级</a>
            <a href="?{if !empty($_REQUEST['search'])}search[level]=3&{$_data['other']['urls']}{else}search[level]=3{/if}{if isset($_REQUEST['handle_type'])}&handle_type={$_REQUEST['handle_type']}{/if}" class="btn {if $_REQUEST['search']['level']==3}btn-success{/if} btn-default">B级</a>
            <a href="?{if !empty($_REQUEST['search'])}search[level]=0&{$_data['other']['urls']}{else}search[level]=0{/if}{if isset($_REQUEST['handle_type'])}&handle_type={$_REQUEST['handle_type']}{/if}" class="btn {if isset($_REQUEST['search']['level']) && $_REQUEST['search']['level']==0}btn-success{/if} btn-default">C级</a>
            <a href="?{if !empty($_REQUEST['search'])}search[level]=-1&{$_data['other']['urls']}{else}search[level]=-1{/if}{if isset($_REQUEST['handle_type'])}&handle_type={$_REQUEST['handle_type']}{/if}" class="btn {if $_REQUEST['search']['level']==-1}btn-success{/if} btn-default">D级</a>
            <a href="?{if !empty($_REQUEST['search'])}search[level]=-2&{$_data['other']['urls']}{else}search[level]=-2{/if}{if isset($_REQUEST['handle_type'])}&handle_type={$_REQUEST['handle_type']}{/if}" class="btn {if $_REQUEST['search']['level']==-2}btn-success{/if} btn-default">E级</a>
        </div>
        <ul class="nav nav-tabs" style="overflow: inherit; padding-left: 100px">
            <li class="{if $_data['other']['handle_type']=='article_count'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}handle_type=article_count&{$_data['other']['urls']}{else}&handle_type=article_count{/if}{if isset($_REQUEST['search']['level'])}&search[level]={$_REQUEST['search']['level']}{/if}">文章数量↓</a>
            </li>
            <li class="{if $_data['other']['handle_type']=='deleted'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}handle_type=deleted&{$_data['other']['urls']}{else}&handle_type=deleted{/if}{if isset($_REQUEST['search']['level'])}&search[level]={$_REQUEST['search']['level']}{/if}">已删除</a>
            </li>
            <li class="{if $_data['other']['handle_type']=='offline'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}handle_type=offline&{$_data['other']['urls']}{else}&handle_type=offline{/if}{if isset($_REQUEST['search']['level'])}&search[level]={$_REQUEST['search']['level']}{/if}">已下线</a>
            </li>
            <li class="{if $_data['other']['handle_type']=='online'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}handle_type=online&{$_data['other']['urls']}{else}&handle_type=online{/if}{if isset($_REQUEST['search']['level'])}&search[level]={$_REQUEST['search']['level']}{/if}">已上线</a>
            </li>
            <li class="{if $_data['other']['handle_type']=='all'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}handle_type=all&{$_data['other']['urls']}{else}&handle_type=all{/if}{if isset($_REQUEST['search']['level'])}&search[level]={$_REQUEST['search']['level']}{/if}">全部</a>
            </li>
        </ul>
    </div>
    <div class="portlet-body">
            ...
            <table class="table table-striped table-bordered table-hover" id="data_table" style="word-break: break-all">
                ...
                {foreach $_data['rows'] as $k=>$v}
                    <tr data-sid="{$v['id']}">
                       ...
                       <td>
                           {if $v['category_id']!=200 && $v['category_id']!=300 && $v['category_id']!=400}
                               <a href="javascript:;" class="btn btn-primary btn-xs list_size warn"><i
                                           class="fa fa-eye"></i>查看</a>
                               <a href="?action=case&space_id={$v['id']}" class="btn btn-info btn-xs list_size"><i
                                           class="fa fa-eye"></i>文章</a>
                                {if $v['status']==-1}
                                <a href="javascript:;" data-status="0"
                                   class="btn btn-success btn-xs list_size warn"><i class="fa fa-eye"></i>恢复</a>
                                {else}
                                <a href="javascript:;" data-status="-1"
                                   class="btn btn-danger btn-xs list_size warn"><i class="fa fa-eye"></i>删除</a>
                                {/if}
                           {else}
                               <a href="?action=detail&space_id={$v['id']}&tab=1" class="btn btn-primary btn-xs list_size"><i
                                           class="fa fa-eye"></i>查看</a>
                               <a href="?action=case&space_id={$v['id']}" class="btn btn-info btn-xs list_size"><i
                                           class="fa fa-eye"></i>文章</a>
                               {if $v['status']==-1}
                               <a href="javascript:;" onclick="OperateSpacepedia.delSpacepedia(this)" data-status="0"
                               class="btn btn-success btn-xs list_size"><i class="fa fa-eye"></i>恢复</a>
                               {else}
                               <a href="javascript:;" onclick="OperateSpacepedia.delSpacepedia(this)" data-status="-1"
                               class="btn btn-danger btn-xs list_size"><i class="fa fa-eye"></i>删除</a>
                               {/if}
                           {/if}
                            <a href="?action=assign&space_id={$v['id']}" class="btn btn-info btn-xs list_size"><i
                                        class="fa fa-eye"></i>分配手机号</a>
...
<script>
    ...
    OperateSpacepedia = {
        ...
        delSpacepedia: function (e) {
            var status = $(e).attr('data-status');
            if (status == -1 && !confirm('您确定要删除吗？？')) {
                return false;

            }
            var space_id = $(e).parents('tr').data('sid');
            $.get('http://{Core::$urls['host']}/static/api/spacepedia/handle.jsonp', {
                status: status,
                space_id: space_id,
                action: 'change_status'
            }, function (data) {
