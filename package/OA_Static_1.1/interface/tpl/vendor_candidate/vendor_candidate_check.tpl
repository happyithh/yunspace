            ...
            <div class="portlet-body">
                <div class="table-responsive clearfix">
                    <table class="table table-striped table-bordered table-hover" id="data_table"
                        ...
                        {*<td>#{$_data[0]['id']}</td>*}
                        {*<td>{$_data[0]['product_name']}{$p_message}</td>*}
                        {*<td>{$_data[0]['vendor_name']}{$v_message}</td>*}
                        {*<td>{$_data[0]['phone']}</td>*}
                        {*<td>{$_data[0]['city']}</td>*}
                        {*<td>{$_data[0]['product_des']}</td>*}
                        {*<td>{$_data[0]['product_addr']}</td>*}
                        {*<td>{$_data[0]['username']}</td>*}
                        {*<td>{if $_data[0]['status']==1}已处理{elseif $_data[0]['status']==2}重复{elseif $_data[0]['status']==4}无效{elseif $_data[0]['status']==5}数据不完整{else}待处理{/if}</td>*}
                        {*<td>{if $_data[0]['from_where']==1}采集{elseif $_data[0]['from_where']==2}表格导入{/if}</td>*}
                        {*<td>{date('Y-m-d H:i:s',$_data[0]['create_time'])}</td>*}
                        {*</tr>*}
                        {*</tbody>*}
                        <tbody >
                        <tr><th style="width:7%">产品ID</th><td>#{$_data[0]['id']}</td></tr>
                        <tr><th>产品名称</th><td>{$_data[0]['product_name']}{$p_message}&nbsp;&nbsp;&nbsp;&nbsp;<a href="https://www.baidu.com/s?wd={$_data[0]['product_name']}" target="_blank">在百度快速搜索该产品</a></td></tr>
                        {*<tr><th>供应商名称</th><td>{$_data[0]['vendor_name']}{$v_message}</td></tr>*}
                        <tr><th>联系电话</th><td>{$_data[0]['phone']}</td></tr>
                        <tr><th>城市</th><td>{$_data[0]['city']}</td></tr>
                        <tr><th>产品描述</th><td>{$_data[0]['product_des']}</td></tr>
                        <tr><th>地址</th><td>{$_data[0]['product_addr']}</td></tr>
                        {if $_data[0]['status']!=3}
                ...
                <div class="table-responsive clearfix">
                    <table class="table table-striped table-bordered table-hover" id="data_table"
                        ...
                        {foreach $p_data as $k=>$v}
                            <tr role="row" data-pid="{$v['id']}">
                                <td>#{$v['id']}</td>
                                <td><a href="?action=product_detail&product_id={$v['id']}" target="_blank">{$v['product_name']}</a></td>
                                <td><a href="?action=vendor_detail&vendor_id={$v['vendor_id']}" target="_blank">#{$v['vendor_id']}--{$v['vendor_name']}</a></td>
            ...
            <div class="portlet-body form-horizontal">
                <div class="form-body" style="min-height: 350px;">
                    ...
                    {else}
                        ...
                        <div class="form-actions text-center">
                            <a class="btn btn-success getCategory" href="javascript:;" id="{$_REQUEST['id']}" num="{$_REQUEST['num']}">完善候选数据</a>
                            <a href="?action=check&handle_type=repeat&id={$_data[0]['id']}&num={$_REQUEST['num']}" class="btn btn-primary">标记为重复</a>
                            <a href="?action=check&handle_type=less&id={$_data[0]['id']}&num={$_REQUEST['num']}" class="btn btn-primary" style="background-color:#FFAA33;border: 1px solid #FFAA33">数据不完整</a>
                            <a href="?action=check&handle_type=invalid&id={$_data[0]['id']}&num={$_REQUEST['num']}"  class="btn btn-danger">标记为无效</a>
                        </div>
                    {/if}
                </div>
            </div>
...
<script>
    jQuery(document).ready(function () {
        ...
        $('.getCategory').click(function () {
            ...
            var num = $('.getCategory').attr('num');
//            var product_name=$('.product_name').val();
            if (!category_id) {
                return false;
            }
            if(!vendor_id){
                window.location.href = '?action=detail&id=' + vendorcandidate_id +'&category_id=' + category_id+'&num='+num;
            }
