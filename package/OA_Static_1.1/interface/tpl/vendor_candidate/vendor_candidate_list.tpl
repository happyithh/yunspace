            ...
            <div>
                <form action="?" method="get" class="clearfix">
                    ...
                    <div class="form-body clearfix">
                        ...
                        <div class="form-group col-md-5">
                            <label class="col-md-3 control-label">城市搜索</label>

                            <div class="col-md-9">
                                <input type="text" class="form-control input-sm" placeholder="请填写城市" name="search[city]" value="{$_REQUEST['search']['city']}">
                            </div>
                        </div>
                    </div>
...
<div class="portlet">
    ...
    <div class="portlet-tabs" style="position:relative">
        {for $i=1;$i<=20;$i++}
        {*<div style="position: absolute;left:{($i-1)*50+10}px;bottom:0px;color: #444444;font-size: 16px"><a href="?num={$i}">{$i}组</a></div>*}
            <div style="position: absolute;left:{($i-1)*50+10}px;bottom:0px;color: #444444;font-size: 16px;{if $_REQUEST['num']==$i}background-color:#AAAAAA;{/if}"><a href="?num={$i}" >{$i}组</a></div>
        {/for}
            ...
            <table class="table table-striped table-bordered table-hover" id="data_table" style="word-break: break-all">
                ...
                {foreach $_data['rows'] as $k=>$v}
                    <tr role="row" data-pid="{$v['id']}">
                        ...
                        {/if}
                        <td><a href="?action=check&id={$v['id']}&num={$_REQUEST['num']}" class="btn btn-success btn-xs"><i class="fa fa-pencil"></i>{if $v['status']==3}处理{else}查看{/if}</a>
