    ...
    <div class="portlet-body">
        <div class="container-fluid">
            <div class="row form-horizontal">
                <form action="?action=check_product&handle_type=refuse&id={$data['id']}&vendor_id={$_REQUEST['vendor_id']}&admin_name={$_REQUEST['admin_name']}" method="post">
                    ...
                    <div class="form-group">
                        <div class="form-actions text-center">
                            <a href="?action=check_product&handle_type=pass&id={$data['id']}&vendor_id={$_REQUEST['vendor_id']}&admin_name={$_REQUEST['admin_name']}&upstatus=1" class="btn btn-success">通过并上线</a>
                            <a href="?action=check_product&handle_type=pass&id={$data['id']}&vendor_id={$_REQUEST['vendor_id']}&admin_name={$_REQUEST['admin_name']}" class="btn btn-primary">通过</a>
                            <button type="submit" class="btn btn-danger">拒绝</button>
                            <a target="_blank" href="http://{Core::$urls['host']}/service_info/{md5($data['id'])}/{$data['id']}.html" class="btn btn-default">新窗口预览</a>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div style="border: 1px solid #ddd">
            <iframe width=100% height=900 frameborder="no" border=0 scrolling=auto src='http://{Core::$urls['host']}/service_info/{md5($data['id'])}/{$data['id']}.html'></iframe>
        </div>
    <div>
</div>
