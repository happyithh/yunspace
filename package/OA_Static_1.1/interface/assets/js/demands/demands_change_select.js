demand = {
    ...
    step: '',
    getAdmin: function (e) {
        var response_account_id = $(e).find('option:selected').val();
        ...
        YunSpace.api('demand/allot.jsonp', {
            demand_id: demand_id,
            admin_id: admin_id,
            response_account_id: response_account_id
        }, function (data) {
            if (data.status == 1) {
                top.alert(data.msg);
                var adminUsername = $(e).find('option:selected').text();
                if (adminUsername != '分配负责人') {
                    $(e).parents('td').html('<a href="?search[response_account_id]=' + response_account_id + '">' + adminUsername + '</a>');
                }
            } else {
                top.alert(data.msg);
            }
