...
{*<div class="submit-success">*}
    ...
    <div class="hide">
        <input type="hidden" class="demand_id" value="{$demand_id}" />
        {foreach $question as $k=>$v}
            <div class="home_question {if $k>0}hide{/if}">
                <p>{$k+1}、{$v['q']}</p>
                <input type="hidden" name="q_num" class="q_num" value="{$k+1}">
                {foreach $v['a'] as $kk=>$vv}
                    <div class="radio" data-name="answer{$k+1}">
                        <input type="radio" name="answer{$k+1}" class="ans" value="{$vv}_{$kk+1}" >
                        <label for="question1">&nbsp;&nbsp;{$vv}</label>
                    </div>
                {/foreach}
            </div>
        ...
        {*<div class="home_question hide">*}
            ...
            {*<div class="radio">*}
                ...
                {*<label for="question1">&nbsp;&nbsp;商场</label>*}
            {*</div>*}
            {*<div class="radio">*}
                {*<input type="radio" name="envir_ok2" class="ans" id="question2">*}
                {*<label for="question2">&nbsp;&nbsp;演艺中心</label>*}
            {*</div>*}
        {*</div>*}
        {*<div class="home_question hide">*}
