...
<div class="side-find" yun="find.v3.0/find-right.js::init">
            ...
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    <a href="/raiders-info/{$val['id']}.html">
                        ...
                        <div class="swiper-slide-title">{mb_strimwidth($val['title'],0,28,'','UTF-8')}</div>
                        <div class="swiper-slide-des">{mb_strimwidth($val['des'],0,60,'…','UTF-8')}</div>
                    </a>
                </div>
                {/foreach}
            </div>
    ...
    <!--广告位开始-->
    <div yun="pa/pa.js::add" yun-name="Find ListA"></div>
    <!--广告位结束-->
    <div class="side-top">
        <div class="side-top-head">
            <div class="side-top-p fl side-top-p-border1 side-top-p-active">周浏览排行</div>
            <div class="side-top-p fl side-top-p-border2">周点赞排行</div>
        </div>

        <div class="side-top-list">
            <ul class="top-list">
                {foreach $page->getWeekTopView as $key=>$val}
        ...
        <div class="side-top-list none">
            <ul class="top-list">
                ...
                {foreach $page->getWeekTopPraise as $key=>$val}
                    <li><a href="/raiders-info/{$val['id']}.html"><i>{$key+1}</i>&nbsp;&nbsp;&nbsp;
                            {if mb_strwidth($val['title'],'UTF-8')>32}
                                {mb_strimwidth($val['title'],0,32,'','UTF-8')}
                            {else}
                                {$val['title']}
                            {/if}
                            &nbsp;<span>{$val['counter_praise']}</span></a></li>
                {/foreach}
            </ul>
        </div>
    </div>
    ...
    <!--广告位开始-->
        <div yun="pa/pa.js::add" yun-name="Find ListB"></div>
    <!--广告位结束-->
