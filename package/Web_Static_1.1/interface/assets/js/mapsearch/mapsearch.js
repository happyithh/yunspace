...
//
////预览展示的显示/隐藏
//$('.li-preview').mouseenter(function(){
//
//   var id =  $(this).attr('data-id');
//    YunSpace.api(
//        'map/search/preview.jsonp', {
//            id: id
//        }, function (data) {
//            var previewHTML = data.data;
//
//            $("#preview").html(previewHTML);
//            $('.m-preview').stop().fadeIn();
//        }
//    );
//});
$('.mcont-list-cont').mouseleave(function(){
    a=setTimeout(function(){
        $('.m-preview').fadeOut();
    },300);

    $('.m-preview').hover(function(){
        clearTimeout(a);
    },function(){
        $('.m-preview').stop().fadeOut();
    });

    //这个是关闭预览的x按钮，放在这里面才管用
    $('.js-mclose').click(function(){
        $('.m-preview').fadeOut();
    });
});
...
$('.js-collect').click(function(){
    if($(this).find('.icon-collect').hasClass('colt1')){
        $(this).find('.icon-collect').removeClass('colt1').addClass('colt2');
    }else{
        $(this).find('.icon-collect').removeClass('colt2').addClass('colt1');
    }
});


//单行活动列表收起张开
$(".a_downup").click(function(){

    if($(this).parent().hasClass('hauto')){
        $(this).parent().removeClass("hauto");

        $(this).removeClass("a_up");
        $(this).addClass("a_down");
    }else{
        $(this).parent().addClass("hauto");
...
//菜单的显示与隐藏
var ddList = $(".dlist").last().index();
if (ddList > 3) {
    $(".dlist").eq(3).addClass('hide');
    $(".dlist").eq(4).addClass('hide');
}
//活动列表收起张开
...
$(document).on('mouseenter',"#listContent ul li",function(){
    var id = $(this).data('id');
    //var c = $(allFieldPoint[id].getContent()).addClass("pton")[0];
    if (allFieldPoint[id]){
        ...
        //    var point = allFieldPoint[id].getPosition();
...
$(document).on('mouseenter','#map .pt',function(){
    $(this).addClass('pton');

});
$(document).on('mouseleave','.pt',function(){
...
$("#pointList ul li").on("click",function(){
...
//$.ajax({
//    type:"get",
//    url:"static/api/map/search/spaceList.json",
//    dataType:'json',
//    success:function(data){
//        allFieldsData = data['data'];
//        //groupDrawing(fieldsData);//分批次渲染
//        drawingOnce(allFieldsData);//一次渲染
//        $('#loadingImg').hide();
//    },
//    error:function(XMLHttpRequest, textStatus, errorThrown){
...
function groupDrawing(fieldsData){//分批次渲染
    var fieldsDataLength = fieldsData.length;
    var time = Math.floor(fieldsDataLength/200);//分批次渲染次数
    var alreadyDrewTime = 0;//已渲染的次数

    for(var i = 0; i < 200; i++){
        drawingPoint(fieldsData[i]);
    }

    var drawingInterval = setInterval(function(){
        alreadyDrewTime =  alreadyDrewTime + 1;
        if (alreadyDrewTime >= time){
            clearInterval(drawingInterval);
        }
        if (alreadyDrewTime == time){
            for(var i = 200 * alreadyDrewTime; i < fieldsDataLength; i++){
                drawingPoint(fieldsData[i]);
            }
        }else{
            for(var i = 200 * alreadyDrewTime; i < (200 * alreadyDrewTime + 200); i++){
                drawingPoint(fieldsData[i]);
            }
        }
    },600);
}
function drawingOnce(fieldsData){
    var fieldLenth = getObjectLength(fieldsData);
    for(var i = 0; i < fieldLenth; i++){
        drawingPoint(fieldsData[i]);
    }
}
...
function drawingPoint(pointData){
    ...
    //htm = '<a class="pt-wrap" href='+url+' target="_blank" id='+pointData['id']+'>'
    //    + '<div class="pt"></div>'
    ...
    //var K = $(rm.K);
    //var kWid = K.find('.pt-detail').width();
    //K.find('.pt-detail').css({
    //    'left': -kWid/2 + 2
    //})
    //$(rNode).css({
    //    'left': -rNode.offsetWidth/2+13
    //})
...
function drawingAreaData(data){
    ...
    for(var i= 0;i < data.length;i++){
        ...
        rm.addEventListener('click',function(e){
            ...
            //var position = allAreaData[area_name]
