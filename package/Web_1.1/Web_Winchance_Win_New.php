 ...
 * Time: 10:16
 */
class Web_Winchance_Win_New extends Page_Base
{
    //文创活动第一横排大图
...
//            不要连表查询
        ...
        $cols = "{table}.*,{prefix}baike_statistical.click,{prefix}baike_statistical.is_love";
        $table = "{table} LEFT JOIN {prefix}baike_statistical ON {table}.space_id={prefix}baike_statistical.space_id";
        $data = Data_Mysql_Table_Winchance_New::select($cols, $condition, '', $num, $table);
        if (empty($data)) {
            return array();
        }
        foreach($data as $k =>$v){
