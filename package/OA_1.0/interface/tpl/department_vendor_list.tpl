            ...
            <div>
                <form action="?" method="get" class="clearfix">
                    <div class="form-body clearfix">
                        ...
                        {*<div class="col-md-9">*}
                        {*<select class="form-control input-sm" name="search[create_by_quick]">*}
                        {*<option value="">全部</option>*}
                        {*<option value="0">批量导入</option>*}
                        {*<option value="1">快速创建</option>*}
                        {*</select>*}
                        {*</div>*}
                        {*</div>*}
                        <div class="form-group col-md-5">
...
<div class="portlet">
    ...
    <div class="portlet-tabs">
        <ul class="nav nav-tabs" style="overflow: inherit; padding-left: 100px">
            <li class="{if $create_type=='quick'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}create_type=quick{$urls}{else}create_type=quick{/if}">快速创建</a>
            </li>
            <li class="{if $create_type=='other'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}create_type=other{$urls}{else}create_type=other{/if}">正常创建</a>
            </li>
            <li class="{if $create_type=='all'}active{/if}">
                <a href="?{if !empty($_REQUEST['search'])}create_type=all{$urls}{else}create_type=all{/if}">全部</a>
            </li>
        </ul>
    </div>
    <div class="portlet-body">
            ...
            <table class="table table-striped table-bordered table-hover" id="data_table" style="word-break: break-all">
                ...
                {foreach $_data['rows'] as $k=>$v}
                    <tr>
                        ...
                        <td>
                            {if $_REQUEST['department']<2&&($v['status']==2 || $v['status']==4)}
                                <a href="?action=waiting&vendor_id={$v['vendor_id']}&admin_name={$_adminInfo[$v['admin_id']]}"
