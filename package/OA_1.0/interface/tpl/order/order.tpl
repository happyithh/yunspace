        ...
        <div>
            <form action="?" method="get">
                ...
                <div style="width: 420px;float: left;margin: 10px 0 10px 0;">
                    <span style="font-size:14px;display: block;float: left;width: 110px;line-height: 40px;height: 30px;">请选择时间段：</span>
            ...
            <table class="table table-striped table-bordered table-advance table-hover">
                ...
                {foreach $data as $v}
                    <tr>
                        ...
                        <td>
                            <a href="?action=user&userid={$v['vendor_account_id']}&name={$v['vendor_name']}&tab=2{if !empty($_REQUEST['start_time'])||!empty($_REQUEST['end_time'])||(!empty($_REQUEST['start_time']) && !empty($_REQUEST['end_time']))}&start_time={$_REQUEST['start_time']}&end_time={$_REQUEST['end_time']}{/if}">{$v['vendor_name']}</a>
                        </td>
                        <td title="{$v['remark']}">
