<form action="?action=contacts" method="post" class="clearfix form-horizontal">
<input type="hidden" name="id" value="{$data['id']}"/>
<input type="hidden" name="is_submit" value="1"/>
<input type="hidden" name="is_my" value="{$_REQUEST['is_my']}"/>
...
{if $data['id']}
<div class="clearfix">
    <button type="submit" class="btn btn-warning" style="width: 150px; float: right">
        确认修改
    </button>
</div>
{/if}
<div class="clearfix">
    <div class="form-group col-md-6">
        ...
        <div class="col-md-9" id="type_but">
            <label class="radio-inline">
                <input type="radio" name="data[type]" value="1" {if $data['type']!=21}checked{/if}>
                客户
            </label>
            <label class="radio-inline">
                <input type="radio" name="data[type]" value="21" {if $data['type']==21}checked{/if}>
                供应商
            </label>
        </div>
    </div>
    ...
    {*客户类型 KV 或 标准*}
        <div class="{if $data['type']==21}display-none{/if} " style="width: 100%;" id="customerType" >
            <div class="form-group col-md-6" >
                ...
                <div class="col-md-9">
                    <label class="radio-inline">
                        <input type="radio" name="data[customer_type]" value="2" {if $data['customer_type']!=1}checked{/if} >
                        标准产品客户
                    </label>
                    <label class="radio-inline">
    ...
    <div class="form-group col-md-6">
        ...
        <div class="col-md-9">
            <input type="text" class="form-control input-sm" placeholder="手机或电话-例：13888888888/0218888888" name="data[手机号]" value="{$data['phone']}">
        </div>
    </div>
    <div class="form-group col-md-6">
        ...
        <div class="col-md-9">
            <select class="form-control input-sm" name="data[业务类型]" id="business" value="{$data['business']}">
                {foreach $categoryList[0] as $key=>$value}
                    {if $value}
                    <option value="{$key}{$value}" {if $key== intval($data['business'])}selected {/if}>{$key}{$value}</option>
                    {/if}
                {/foreach}
            </select>
        </div>
                    ...
                    {foreach $categoryList as $ck=>$cv}
                        {if intval($data['business'])==$ck}
                            {foreach $cv as $cvk=>$cvv}
                                <option value="{$cvk}{$cvv}" {if $cvk==intval($data['sub_business'])}selected {/if}>{$cvk}{$cvv}</option>
                            {/foreach}
                        {/if}
                    {/foreach}
                </select>
...
<div class="clearfix">
    <div class="form-group col-md-6">
        ...
        <div class="col-md-9">
            <input type="text" class="form-control input-sm" placeholder="公司名称" name="data[公司名称]" value="{$data['company']}">
        </div>
    </div>
    <div class="form-group col-md-6">
    ...
    <div class="form-group col-md-6">
        ...
        <div class="col-md-9">
            <input type="text" class="form-control input-sm" placeholder="公司网站" name="data[attr][公司信息][公司网站]" value="{$data['attr']['公司信息']['公司网站']}">
        </div>
    </div>
    <div class="form-group col-md-6">
