    ...
    <div class="portlet-body">
        <blockquote class="hero">
            ...
            {if ($data['is_my'])||$data['is_admin']}
                ...
                {else}
                    <span style="float: right;margin-right: 10px;">
                    <a data-id="{$data['id']}" class="btn btn-danger" href="javascript:;"
                       onclick="ListOperator.unbind_account(this)" style="color: #fff">解除绑定</a>
                </span>
                {/if}
            {/if}
            {if $data['is_admin']}
                <span style="float: right;margin-right: 10px;">
                <a class="btn btn-primary" href="javascript:;" style="color: #fff"
                   onclick="ListOperator.change_receiver(this)">
                    分配负责人
                </a>
            </span>
        ...
        <div class="tab-content clearfix">
            {if $_REQUEST['tab']==2}
                {include file="contacts/inc/contact_detail_tab2.tpl"}
            {elseif $_REQUEST['tab']==3}
                {if $_REQUEST['demand_id']}
                    {include file="customer_account/inc/tab_demand_recommend.tpl"}
                {else}
                    {include file="contacts/inc/contact_detail_tab3.tpl"}
                {/if}
            ...
            {elseif $_REQUEST['tab']=='bind'&&($data['is_my']||$data['is_admin'])}
                {include file="contacts/inc/contact_detail_tab_bind.tpl"}
            {else}
                {include file="contacts/inc/contact_detail_tab1.tpl"}
            {/if}
        </div>
    </div>
</div>
