    ...
    <div class="portlet-tabs">
        <ul class="nav nav-tabs" style="overflow: inherit; padding-left: 100px">
            <li class="{if $page_nav=='delete'}active{/if}">
                <a href="?{if !empty($_REQUEST['search']['title'])}action=listDelete&{$urls}{else}action=listDelete{/if}">已删除</a>
            </li>
            <li class="{if $page_nav=='stop'}active{/if}">
                <a href="?{if !empty($_REQUEST['search']['title'])}action=listStop&{$urls}{else}action=listStop{/if}">已停用</a>
            </li>

            <li class="{if $page_nav=='normal'}active{/if}">
                <a href="?{if !empty($_REQUEST['search']['title'])}action=normal&{$urls}{else}action=normal{/if}">正常</a>
            </li>
