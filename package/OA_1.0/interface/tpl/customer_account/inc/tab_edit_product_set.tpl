<div class="portlet tabbable">
    ...
    <form class="form-horizontal" role="form" action="?" method="post">
        <div class="portlet-body form">
            ...
            <input type="hidden" name="set_add" value="{$_REQUEST['set_add']}">
            <input type="hidden" name="tab" value="{$_REQUEST['tab']}">
            <input type="hidden" name="id" value="{$_REQUEST['id']}">
            <input type="hidden" name="vendor_id" value="{$_data['vendor_id']|default:$_REQUEST['vendor_id']}">
            <style>
...
<script>
    ...
    $("#schedule_product_id").select2({
        ...
        ajax: {
            url: "?action=detail&id={$_REQUEST['id']}&tab={$_REQUEST['tab']}",
            dataType: 'json',
        ...
        initSelection: function (element, callback) {
            var id = $(element).val();
            if (id !== "") {
                $.ajax("?action=detail&id={$_REQUEST['id']}&tab={$_REQUEST['tab']}&query_product=%QUERY", {
                    data: {
                        product_id: id
                    },
                    dataType: "json"
