            ...
            <div>
                <ul class="nav nav-tabs">
                    ...
                    <li{if $_REQUEST['type']=='is_check'} class="active"{/if}>
                        <a href="?type=is_check">审核通过</a>
                    </li>
                    <li{if $_REQUEST['type']=='is_error'} class="active"{/if}>
                        <a href="?type=is_error">未通过</a>
                    </li>
                </ul>
            </div>
            <style>
            ...
            <table class="table table-striped table-bordered table-hover" id="data_table" style="word-break: break-all">
                ...
                {foreach $list as $k=>$v}
                    <tr role="row">
                        ...
                        <td>{$v['detail']['bank_account']}
                            ({$v['detail']['username']})
                        </td>
                        <td>{$v['detail']['bank_name']}
                        ...
                        <td>{$v['category']}</td>
                        <td>
                            <a href="?id={$v['create_time']}-{$v['id']}&action=check" class="btn btn-primary btn-xs"><i class="fa fa-eye"></i>
                                {if $v['confirm_time']>0}
                                    查看
                                {else}
                                    审核
