                ...
                <table class="table table-hover" style="border:0px;">
                    ...
                    <tr>
                        ...
                        <td>
                            {foreach $_demand['demand'] as $key=>$val}
                                {if is_array($val)}
                                    ...
                                    {else}
                                        ...
                                        <p>&nbsp;&nbsp;&nbsp;
                                        {foreach $val as $v}
                                            <a  target="_blank" href="http://{Core::$urls['host']}/site/{Func_Url::str_urlencode($v)}-百科">{$v}</a>;
                                        {/foreach}
                                        </p>
                                    {/if}
                                {else}
                                    <p>{$key}：&nbsp;&nbsp;&nbsp;
                                        {if $key=="意向场地空间"}
                                            <a target="_blank" href="http://{Core::$urls['host']}/site/{Func_Url::str_urlencode($_demand['demand']['意向场地'],1)}-{Func_NumEncode::e($_demand['demand']['item_id'])}">{$val}</a> 
                                        {elseif $key=="意向场地"}
                                            <a target="_blank" href="http://{Core::$urls['host']}/site/{Func_Url::str_urlencode($val)}-百科">{$val}</a>                                    
                                        {elseif $key=="免费vip-名片"}
                                            <a target="_blank" href="http://{Core::$urls['host']}{str_replace(",",'',$val)}">点此查看名片详情</a>
                                        {else}
                                            {$val}
                                        {/if}
                                    </p>
                            ...
                            {if $events_data}
                                <p>要咨询的场地：&nbsp;&nbsp;&nbsp;
                                    {foreach $events_data as $k=>$v}
                                        [{$v['object_id']}]:
                                        {if $v['object_type']=="site"}
                                            <a target="_blank" href="http://{Core::$urls['host']}/site/{Func_Url::str_urlencode($v['object_name'])}-百科">{$v['object_name']}</a>；
                                        {elseif $v['object_type']=="vendor"}
                                            <a target="_blank" href="http://{Core::$urls['host']}/vendor_info/{$v['object_id']}.html">{$v['object_name']}</a>；
                                        {else}
                                            {$v}；
                                        {/if}
                                    {/foreach}
                            ...
                            {/if}
                            {if !empty($_demand['demand']['子空间ID'])}
                                <p>要咨询的空间：&nbsp;&nbsp;&nbsp;

                                    <a target="_blank" href="http://{Core::$urls['host']}/site/{Func_Url::str_urlencode($v['object_name'])}-百科-{Func_NumEncode::e($_demand['demand']['子空间ID'])}">{$_demand['demand']['子空间名称']}</a>；
