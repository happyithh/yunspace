...
//                            alert('结束时间必须大于开始时间');
            ...
            {*$("#datetimepicker_input1").focus(function(){*}
                {*var days={$page->info['attr_index']['天数']};*}
                {*var date1=$(this).val();*}
                {*if(date1){*}
                    {*date1 = date1.replace(/-/g,'/');*}
                    {*var time1 = new Date(date1);*}
                    {*var times1 = time1.getTime();*}
                    {*var times2=times1*days*24*3600;*}
                    {*alert(times2);*}
{*//                    $('#showDate').fullCalendar( 'select', $("#datetimepicker_input1").val(), $("#datetimepicker_input2").val(times2))*}
{*//                    $("#datetimepicker_input2").val();*}
{*//                    alert(times2);*}
                {*}*}
            {*})*}
