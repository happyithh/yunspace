...
class OA_Demand_All_Detail
{
    static function  handleDemandInfo($department, $is_team, $is_all, $is_my)
    {
        ...
        Core::log('demand_info_log_by_adu','需求详情页',"用户名：".$_SESSION['administrator']['fullname'],"需求id:".$request['demand_id'],Func_Input::ip(),$is_my,$_COOKIE['un_a']);
        $col = "t1.*,t2.step as grab_step,t2.sale_phone,t2.customer_phone";
        $table = "{table} as t1 LEFT JOIN {prefix}grab_order as t2 on t1.id=t2.id";
        //Data_Mysql_Table_Demand::debug(1);
        $demand_data = Data_Mysql_Table_Demand::select($col, array('t1.id =?' => $request['demand_id']), '', 1, $table);
        if (!empty($demand_data)) {
            foreach ($demand_data as $k => $v) {
                $demand_data[$k]['demand'] = json_decode($v['demand'], 1);
                $demand_data[$k]['backup_demand'] = json_decode($v['backup_demand'], 1);
            }
        }
        // q($demand_data);
        if (!empty($demand_data[0]['sale_phone'])) {
            $admin = Api_Administrator_getBaseInfo::getAdminInfoByPhone(array('phone' => $demand_data[0]['sale_phone']));
            Tpl_Smarty::assign('admin_info', $admin);
        }
        Tpl_Smarty::assign('_step_funnel', Data_Mysql_Table_Demand::$step_funnel);
        Tpl_Smarty::assign('_demand', $demand_data[0]);
        ...
        //联系记录;
        /*      $args = array('id' => $request['demand_id'], 'phone' => $_SESSION['administrator']['phone']);
              $data = Api_Grab_Order_Get_Info::getInfo($args);
             // q($data);
          */
//        if (!empty($data['data'])) {
//            $contactRecord = Api_Grab_Order_GetContactRecord::handleRecord(array('phone' => $data['data']['customer_phone'], 'demand_id' => $request['demand_id']));
//            Tpl_Smarty::assign('contactRecord', $contactRecord['data']);
//        }
        if (!empty($demand_data[0]['customer_phone'])) {
            $customer_phone = $demand_data[0]['customer_phone'];
        } else {
            $customer_phone = $demand_data[0]['phone'];
        }
        Tpl_Smarty::assign('customer_phone', $customer_phone);
        Tpl_Smarty::assign('contactRecord', self::getRecord(array('phone' => $customer_phone)));

        //q($contactRecord);
        $events_rs = Api_Demand_GetEventsById::getEvents(array('id' => $request['demand_id']));
        $events = $events_rs['data'];
        Tpl_Smarty::assign('events', $events);
        ...
        Tpl_Smarty::display("OA_Static::Demand.v2.0/demand_detail.tpl");
    }
    ...
    static function handleAddContactRecord()
    {
        ...
        if (empty($contact_people[0])) {
            ...
            if (!empty($demand['backup_demand'])) {
                $demand['demand'] = $demand['backup_demand'];
            }
            $contact = array(
...
//        $conditions = array();
//        if (!empty($request['phone'])) {
//            $table = "{table} as t1 LEFT JOIN {prefix}account as t2 on t1.phone=t2.phone";
//            $contact_people = Data_Mysql_Table_Administrator::select('t1.id as admin_id,t2.id as account_id', array('t1.phone =?' => $request['phone']), '', 1, $table);
//            if (!empty($contact_people[0]['admin_id'])) {
//                $conditions['admin_id =?'] = $contact_people[0]['admin_id'];
//            } elseif (!empty($contact_people[0]['account_id'])) {
//                $conditions['contact_id =?'] = $contact_people[0]['account_id'];
//            } else {
//                return false;
//            }
//        }
...
//    }
    static function getRecord($args)
    {
        ...
        if (!empty($request['contact_id'])) {
            $conditions['(contact_id =? OR phone=?)'] = array($request['contact_id'], $request['phone']);
        } else {
            $conditions['phone=?'] = array($request['phone']);
        }
        $record = Data_Mysql_Table_Contact_Record::page(0, 15, '*', $conditions, 'ORDER BY time DESC');;
        foreach ($record['rows'] as $k => $v) {
            $record['rows'][$k]['time'] = date("Y-m-d H:i:s", $v['time']);
