...
class App_Api_Vendor_Main_V2 extends Base
{
    ...
    static function handle()
    {
        ...
        switch (Core::$urls['action']) {
            case 'login':
                self::login();
                break;
            //找回密码 1,  创建时间:2015-6-24 12:07
            case 'retrieve_password_one':
                App_Api_Vendor_Account::retrieve_password_one();

                break;
            //找回密码 3,  创建时间:2015-6-24 12:07
            case 'retrieve_password_three':
                App_Api_Vendor_Account::retrieve_password_three();
                break;
            case 'modification_password':
                App_Api_Vendor_Account::modification_password();
                break;
            case 'user_order_list':
                self::user_order_list();
                break;
            case 'user_order_list_new':
                self::user_order_list_new();
                break;
            case 'order_info':
                self::order_info();
                break;
...
//        }
        ...
        if (!empty($rs[0]) && $rs[0]['password'] == $makePassword) {
            ...
            $data = array(
                ...
                'avatar' => 'http://' . Core::$urls['host'] . Page_Site_Global::displayLogo($rs[0]['avatar'], 100, 100)
            );
...
//       }
        $table = "{table} as t1 LEFT JOIN {prefix}account as t2 ON t1.user_id = t2.id";
        $conditions['t1.user_id =?'] = $request['user_id'];
        $rs = Data_Mysql_Table_App_Login::select('t1.token,t1.login_time,t1.fullname,t1.username,t2.avatar', $conditions, '', 1, $table);
    ...
    static function user_order_list()
    {
        ...
        $rs = Data_Mysql_Table_Order_Info::select('order_no,update_time,id,order_title,order_status,buyer_account_id,vendor_account_id', $condition, 'ORDER BY id DESC', 15);
        if (empty($rs[0])) {
            Func_Output::json('error', '', $msg);
        }
        $data = array();
        if ($rs) {
            $data = $rs;
            foreach ($data as $k => $v) {
                $data[$k]['logo'] = '/package/Web_1.0/interface/assets/img/logo_a.png';
                $data[$k]['order_status_text'] = $status[$data[$k]['order_status']];
                $data[$k]['update_time'] = date("Y-m-d h:i:s", $data[$k]['update_time']);
            }
        }

        Func_Output::json('success', $data, '获取成功！');

    }

    static function user_order_list_new()
    {
        $request = Func_Input::filter(array(
            'user_id' => 'int',
            'from_id' => 'int',
        ...
        $status = Data_Mysql_Table_Order_Info::$order_status_tpl;
        $rs = Data_Mysql_Table_Order_Info::select('order_no,update_time,id,order_title,product_id,product_set_id,order_status,buyer_account_id,vendor_account_id', $condition, 'ORDER BY id DESC', 15);
        if (empty($rs[0])) {
            Func_Output::json('error', '', $msg);
        }
        $data = array();
        foreach ($rs as $k => $v) {
            $ids[] = $v['product_id'];
            $set[] = $v['product_set_id'];
        }
        $product = Data_Mysql_Table_Product_Search::select('logo', array("id IN (" . implode(',', $ids) . ")"));
        $set = Data_Mysql_Table_Product_Set::select('logo', array("id IN (" . implode(',', $set) . ")"));
        if ($rs) {
            $data = $rs;
            foreach ($data as $k => $v) {
                if (!empty($data[$k]['product_id'])) {
                    foreach ($product as $key => $val) {
                        $data[$k]['logo'] = 'http://' . Core::$urls['host'] . Page_Site_Global::displayLogo($product[$key]['logo'], 100, 100);
                    }
                }
                if (!empty($data[$k]['product_set_id'])) {
                    foreach ($set as $key => $val) {
                        $data[$k]['logo'] = 'http://' . Core::$urls['host'] . Page_Site_Global::displayLogo($set[$key]['logo'], 100, 100);
                    }
                }
                //   $data[$k]['logo'] = '/package/Web_1.0/interface/assets/img/logo_a.png';
                $data[$k]['order_status_text'] = $status[$data[$k]['order_status']];
    ...
    static function order_info()
    {
        ...
        if (!empty($data)) {
            Func_Output::json('success', $data, '成功');
        }
        Func_Output::json('error', '', '获取失败');
    }

    static function order_info_new()
    {
        $order_id = $_REQUEST['order_id'];
        $status = Data_Mysql_Table_Order_Info::$order_status_tpl;
        $rs = Data_Mysql_Table_Order_Info::select('order_no,product_id,product_set_id,order_title,start_time,order_status,buyer_account_id,create_time', array('id' => $order_id));
        foreach ($rs as $k => $v) {
            $ids[] = $v['product_id'];
            $set[] = $v['product_set_id'];
        }
        ...
        if (!empty($rs[0]['product_id'])) {
            foreach ($product as $key => $val) {
                $rs[0]['logo'] = 'http://' . Core::$urls['host'] . Page_Site_Global::displayLogo($product[$key]['logo'], 100, 100);
            }
        }
        if (!empty($rs[0]['product_set_id'])) {

            foreach ($set as $key => $val) {
                $rs[0]['logo'] = 'http://' . Core::$urls['host'] . Page_Site_Global::displayLogo($set[$key]['logo'], 100, 100);
            }
        }
        $rs[0]['order_status_text'] = $status[$rs[0]['order_status']];
        //   $rs[0]['logo'] = '/package/Web_1.0/interface/assets/img/logo_a.png';
        $rs[0]['start_time'] = date("Y-m-d h:i:s", $rs[0]['create_time']);
    ...
    static function load_more_order_list()
    {
        ...
        $status = Data_Mysql_Table_Order_Info::$order_status_tpl;
        $rs = Data_Mysql_Table_Order_Info::select('order_no,product_id,product_set_id,update_time,id,order_title,order_status,vendor_account_id', $condition, 'ORDER BY id DESC', 10);
        $data = array();
        $ids = array();
        $set = array();
        foreach ($rs as $k => $v) {
        ...
        Func_Output::json('error', '', '没有数据，请登录云·SPACE官网获取更多操作！');
...
//        $conditions['m.to_account_id = ?'] =$user_id;
        ...
        $data = Data_Mysql_Table_Account::select('a.id,a.avatar, a.username,m.message, MAX(m.send_time) AS send_time, MAX(m.read_time) AS read_time, m.from_account_id, m.to_account_id, sum(read_time=0) as c ', $conditions, ' ORDER BY m.send_time DESC', 35, '{prefix}account AS a
        LEFT JOIN {prefix}message AS m ON (m.from_account_id = a.id or m.to_account_id=a.id)');
        if (empty($data[0])) {
            Func_Output::json('error', '', '登录云·yunsapce网站获取更多操作');
        }
        foreach ($data as $k => $v) {
            if ($data[$k]['id'] == $user_id) {
                ...
                $data[$k]['avatar'] = 'http://' . Core::$urls['host'] . Page_Site_Global::displayLogo($v['avatar'], 100, 100);
                $data[$k]['real_send_time'] = $v['send_time'];
                $data[$k]['send_time'] = Func_Time::format($v['send_time']);
            }

        }
        Func_Output::json('success', array_values($data), '获取成功！');
    }
    ...
    static function get_messages_content()
    {
        ...
        $data = Data_Mysql_Table_Message::select('m.id,m.from_account_id,a.username,a.avatar,m.message,m.send_time,m.to_account_id', $conditions1, 'ORDER BY m.send_time DESC', 15, '{table} AS m
    ...
    static function send_message()
    {
        ...
        Api_Livechat_Add_Contacts::addContacts(array('re_id' => $request['to_id']));
...
//            $backData['avatar'] = 'http://' . Core::$urls['host'] . Core::$urls['root'] . 'user_center/avatar/' . $request['user_id'] . '.jpg';
        ...
        //修改最后一条信息表 adu 2015-07-02 21:09
        $rsml = Data_Mysql_Table_Live_Chat_Message_List::select("*", "(from_account_id = " . $request['user_id'] . " AND to_account_id=" . $request['to_id'] . ") OR (from_account_id = " . $request['to_id'] . " AND to_account_id=" . $request['user_id'] . ")");

        $messageCount = Data_Mysql_Table_Message::select('COUNT(id) as c', array('from_account_id =?  AND to_account_id =? AND read_time =?' =>
            array(
                $request['to_id'],
                $request['user_id'],
                ''
        ...
        if ($rsml) {
            $sql = "UPDATE {table} SET total =?, message = ? ,update_time = " . time() . " WHERE  (from_account_id = ? AND to_account_id=?) OR (from_account_id = ? AND to_account_id=?)";
            $updata = array(
                $messageCount[0]['c'],
                $request['message'],
                $request['user_id'],
            ...
            Data_Mysql_Table_Live_Chat_Message_List::query($sql, $updata, 1);
        } else {
            $indata = array(
                'total' => $messageCount[0]['c'],
                'from_account_id' => $request['user_id'],
                'to_account_id' => $request['to_id'],
                'update_time' => time(),
                'message' => $request['message'],
            ...
            Data_Mysql_Table_Live_Chat_Message_List::insert($indata);
        }

        if (!empty($backData)) {
            Func_Output::json('success', $backData, '发送成功');
        } else {
            Func_Output::json('error', $backData, '发送失败');
        }
...
//        Data_Mysql_Table_Message::update($conditions, $data);
        ...
        $data = Data_Mysql_Table_Message::select('m.id,m.from_account_id,a.username,m.message,m.send_time,m.to_account_id,a.avatar', $conditions1, 'ORDER BY m.send_time DESC', 15, '{table} AS m
        LEFT JOIN {prefix}account AS a ON m.from_account_id = a.id');
    ...
    static function  get_user_admin_money()
    {
        ...
        if (empty($account)) {
            Func_Output::json('error', '', '您需要实名认证之后才能开通我的钱包账户，请登录云·sapce官网进行实名认证！');
        }

        if (isset($request['date'])) {
            $date = strtotime($request['date']);
            $accountDetail = Finance_Account::getDetailList($request['user_id'], $date);
        } else {
            $accountDetail = Finance_Account::getDetailList($request['user_id']);
        }
        ...
        $data['dates'] = $dates;
        if (!empty($data['dates'])) {
            Func_Output::json('success', $data, '成功获取');
        }
        Func_Output::json('error', '', '没有获取到更多数据，请登录云·SPACE官网获取更多操作！');
    }
    //获取账户交易列表---加载更多
    //参数    用户id , limit=0,10
    static function get_account_detail_load_more()
    {
        ...
        if (empty($request['page'])) {
            ...
            $srart = ($request['page'] - 1) * 10;
            $end = ($request['page']) * 10 - 1;
            $limit = "$srart,$end";
        }
        $accountDetail = Finance_Account::getDetailList($request['user_id'], '', '', $limit);
        $data['account_detail'] = $accountDetail;
    ...
    static function  vendorHandleOrder()
    {
        ...
        if (!empty($rs[0]['id']) && $rs[0]['vendor_account_id'] == $request['user_id']) {
            ...
            if (!empty($r[0])) {
                Func_Output::json('success', $r, $msg);

            } else {
                Func_Output::json('error', '', '处理失败！');
            }
        } else {
            Func_Output::json('error', '', '操作异常！');
        }
    }
