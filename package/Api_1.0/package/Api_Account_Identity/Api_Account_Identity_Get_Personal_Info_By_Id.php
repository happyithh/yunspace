 ...
 */
@session_start();
class Api_Account_Identity_Get_Personal_Info_By_Id extends Api
{
//    //个人身份认证信息
...
//    }
    ...
    static function getInfo($args)
    {
        ...
        $rs = Data_Mysql_Table_Account_Identity_Personal_Info::select($cols,array('tb1.id=?'=>$request['identity_id']),'',1,$table);
        if($rs && isset($rs[0])){
            $result['id'] = $rs[0];
