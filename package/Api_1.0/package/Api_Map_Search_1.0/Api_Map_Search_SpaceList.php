 ...
 *初始化地图数据
 */
class  Api_Map_Search_SpaceList extends Api
{
    ...
    static function  SpaceList($args)
    {
        ...
        $data = Data_Mysql_Table_Product_Search::select('id,product_name,geo_lat,geo_lng,logo,attr_index', $conditions, '', 300);
    ...
    static function  ajaxSearch($args)
    {
        ...
        $list = Data_Mysql_Table_Product_Search::page(
            empty($request['p']) ? 0 : $request['p'],
            empty($request['n']) ? 0 : $request['n'],
            'id, category_id,logo,geo_lat,geo_lng, vendor_id,vendor_name, product_name,addr, attr_index',
            $conditions,
            $order
        );
        //处理所有坐标
        ...
        $DataTpl = Tpl_Smarty::fetch("Web_Static::map_search.v1.0/inc/ajax_list.tpl");
    ...
    static function  coordinate($conditions, $order, $page)
    {
        ...
        $list = Data_Mysql_Table_Product_Search::select(
            'id,geo_lat,geo_lng,product_name',
            $conditions,
            $order,
//            $page['dt']
            300
    ...
    static function  conditions($args)
    {
        ...
        if (!empty($request['a_112'])) {
            $conditions['category_id =?'] = $request['a_112'];
        } else {
        ...
        if (!empty($request['a_49'])) {
            $conditions['attr_index'][49] = '+' . trim($request['a_49']);
        }
        ...
        switch ($request['a_10']) {
            case 1:
                ...
                break;
            case 200:
                $conditions['order_product_size >=? AND  order_product_size <=? '] = array(200, 500);
                break;
            case 500:
                $conditions['order_product_size >=? AND  order_product_size <=? '] = array(500, 800);
                break;
            case 800:
                $conditions['order_product_size >=? AND  order_product_size <=?'] = array(800, 1000);
                break;
            case 1000:
                $conditions['order_product_size >=? AND  order_product_size <=?'] = array(1000, 2000);
                break;
            case 2000:
...
//        }
        ...
        if (!empty($request['right_top_lng']) && !empty($request['right_top_lat']) && !empty($request['left_down_lng']) && !empty($request['left_down_lat'])) {
            $request['right_top_lng'] = (float)$request['right_top_lng'] * 1000;
            $right_top_lng = substr($request['right_top_lng'], 0, 6);
            $request['right_top_lat'] = (float)$request['right_top_lat'] * 1000;
            $right_top_lat = substr($request['right_top_lat'], 0, 5);
            $request['left_down_lng'] = (float)$request['left_down_lng'] * 1000;
            $left_down_lng = substr($request['left_down_lng'], 0, 6);
            $request['left_down_lat'] = (float)$request['left_down_lat'] * 1000;
            $left_down_lat = substr($request['left_down_lat'], 0, 5);
    ...
    static function HandleField($list)
    {
        ...
        foreach ($list as $k => $v) {
            ...
            if (!empty($list[$k]['attr_index'][49])&&trim($list[$k]['attr_index'][49])) {
                $activity_type = explode(',', str_replace('、',',',$list[$k]['attr_index'][49]));
            }
