...
var yunspaceMap = function () {
    ...
    self.init = function () {
        ...
        self.options.placeFromLastPage = decodeURI(self.GetQueryString('place'));//上页传过来的地址
        ...
        self.map.disableScrollWheelZoom(true);
        ...
        self.setSelectedCity(self.options.city);

    };
    //设置选中城市的场地
    self.setSelectedCity = function (_city) {
        var cityIndex = 5;
        switch (_city) {
            case '上海':
                cityIndex = 0;
            ...
            case '成都':
                ...
                break;
            case '杭州':
                $(".footer-li").find(".footer-li-header").removeClass('footer-li-header-active');
                $(".footer-li").find(".footer-li-header").eq(5).addClass('footer-li-header-active');
                $(".footer-act-list").addClass("none");
                $(".footer-act-list").eq(5).removeClass('none');
                break;
            default :
                $(".change-city").eq(0).find('li>a').eq(5).attr('value',_city).text(_city);
                $(".change-city").eq(1).find('li>a').eq(5).attr('value',_city).text(_city);
                $(".change-city").eq(2).find('li>a').eq(5).attr('value',_city).text(_city);
                $(".change-city").eq(3).find('li>a').eq(5).attr('value',_city).text(_city);
                $(".change-city").eq(4).find('li>a').eq(5).attr('value',_city).text(_city);
                $(".change-city").eq(0).find('li').eq(5).find('a').click()
                $(".change-city").eq(1).find('li').eq(5).find('a').click();
                $(".change-city").eq(2).find('li').eq(5).find('a').click();
                $(".change-city").eq(3).find('li').eq(5).find('a').click();
                $(".change-city").eq(4).find('li').eq(5).find('a').click();
           }
        if (cityIndex!= -1) {
            $(".change-city").eq(0).find('li').eq(cityIndex).find('a').click();
            $(".change-city").eq(1).find('li').eq(cityIndex).find('a').click();
            $(".change-city").eq(2).find('li').eq(cityIndex).find('a').click();
            $(".change-city").eq(3).find('li').eq(cityIndex).find('a').click();
            $(".change-city").eq(4).find('li').eq(cityIndex).find('a').click();
            $(".footer-li").find(".footer-li-header").removeClass('footer-li-header-active');
            $(".footer-li").find(".footer-li-header").eq(cityIndex).addClass('footer-li-header-active');
            $(".footer-act-list").addClass("none");
            $(".footer-act-list").eq(cityIndex).removeClass('none');
        }
    };
    self.tolerantGetPoint = function () {
        ...
        if (!self.options.cityCenter) {
            ...
            function myFun(r) {
                ...
                $("#city-select-map").find("li").each(function (k, o) {
                    var $o = $(o);
                    var c = $o.find("a").first().text();
                    if (cityName.indexOf(c) > -1) {
                        $("#city-select-map").find("li>a").removeClass("city-active");
                        $o.trigger("click").find("a").addClass("city-active");
                    }
                    //$o.on("click",function(){
                    //    var $this=$(this);
                    //    $("#city-select-map").find("li>a").removeClass("city-active");
                    //    $this.find("a").addClass("city-active");
                    //    $(".cn").text($this.find("a").first().text());
                    //    self.initMapWithPlace( $this.find("a").text()+'市');
                    //});
                });
            }
        ...
        $("#city-select-map").find("li").each(function (k, o) {
            ...
            $o.on("click", function () {
                ...
                $(".cn").text($this.find("a").first().text());
                self.initMapWithPlace($this.find("a").text() + '市');
                self.setSelectedCity($this.find("a").first().text());
            });
        });
        ...
        //                $("#city-select-map").find("li>a").removeClass("city-active");
        //                $this.find("a").addClass("city-active");
        //                $(".cn").text($this.find("a").text());
        //                self.initMapWithPlace( $this.find("a").text());
        //            });
        //        });
        //    }
        //    else{ //定位失败不做处理
    ...
    self.initMapWithPoint = function (point) {
        ...
        //self.map.centerAndZoom(new BMap.Point(116.395645,39.929986),self.mapData.mapZoom);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (!self.isOnlyGetListData && !besePoint) {//是否只更新列表不更新地图
                        ...
                        if (status != 0) {
                            var num = coordinate.length < 20 ? coordinate.length : 20;
                            for (var i = 0; i < num; i++) {
    ...
    self.drawingPoint = function (pointData) {
        ...
        + '<div class="pt-detail">' +
        '<a class="blank" href="' + url + '" target="_blank">' + product_name + '</a>' +
    ...
    self.initSearchInput = function () {
        ...
        var ac = new BMap.Autocomplete(    //建立一个自动完成的对象
            {
                "input": "customPlace", "location": self.map
            });
