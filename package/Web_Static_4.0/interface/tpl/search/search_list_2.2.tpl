...
<head>
    ...
    {include file="Web_Static::v4.0/html_goto_mobile.tpl" url="m/space_new"}
    ...
    {*<script>*}
        ...
        {*$('.search_list_title').each(function(i, block) {*}
{*//            $(block),find("a span");*}
            {*hljs.highlightBlock($(block).find("a span"));*}
        {*});*}
    {*</script>*}
    <style>
        .hide {
    ...
    <div class="search_select clearfix">
        <div class="search_select_content">
            <div class="search_select_section">
                <div class="search_select_section_left fl">
                    空间价格
                </div>
                <ul id="search_industry_type" class="search_select_section_right fl clearfix">
                    <li class="{if empty($page->getKeyword['price_range'])}search_select_active{/if}">
                        <a href="{$_home}space/{$page->getUrls('price_range','')}">不限</a>
                    </li>
                    {foreach $page->getPrice as $k=>$v}
                        <li class="{if $page->getKeyword['price_range']==$k}search_select_active{/if}">
                            <a href="{$_home}space/{$page->getUrls('price_range',$k)}">{$v}</a>
                        </li>
                    {/foreach}
                    <li class="cus-input">
                        <form action="{$_home}space" method="get">
                            <span>自定义</span>&nbsp;&nbsp;
                            {include  file="Web_Static::search/inc/inc.search_keyword.tpl"}
                            <input name="price_range[0]" type="text" value="{$page->customShow['price_range'][0]}"/>&nbsp;-&nbsp;<input
                                    type="text" name="price_range[1]" value="{$page->customShow['price_range'][1]}">&nbsp;元
                            <button type="submit" class="cus-price-ok none">确定</button>
                        </form>
                    </li>
                </ul>
            ...
            <div class="search_select_section clearfix">
                <div class="search_select_section_left">
                    场地类型
                </div>
                <ul id="search_activity_type" class="search_select_section_right clearfix">
                    <li class="{if empty($page->getKeyword['category']) || $page->getKeyword['category']==112}search_select_active{/if}">
                        <a href="{$_home}space/{$page->getUrls('category','')}">不限</a>
                    </li>
                    {foreach $page->getCategory as $k=>$v}
                        {if $k!=112 && ($k<121 || $page->getKeyword['category']==$k)}
                            <li class="{if $page->getKeyword['category']==$k}search_select_active{/if}">
                                <a href="{$_home}space/{$page->getUrls('category',$k)}">{$v}</a>
                            </li>
                        {/if}
                    {/foreach}
                    <li class="more">
                        <a href="javascript:;" class="more-cate">更多</a>
                        <ul class="clearfix">
                            {foreach $page->getCategory as $k=>$v}
                                {if $k>=121 && $page->getKeyword['category']!=$k}
                            <li><a href="{$_home}space/{$page->getUrls('category',$k)}">{$v}</a></li>
                                {/if}
                           {/foreach}
                        </ul>
                    </li>
            ...
            {if count($page->getArea['行政区域'])>0}
            <div class="search_select_section"  {if !empty($page->getKeyword['business_area'])}style="display:none"{/if}>
                <div class="search_select_section_left">
                    行政区域
                </div>
                <ul class="search_select_section_right clearfix" style="{if !empty($page->getKeyword['admin_area'])}height: auto;{/if}">
                    <li {if empty($page->getKeyword['admin_area'])}class="search_select_active"{/if}><a  href="{$_home}space/{Web_Search_Item_List::getUrls('admin_area','')}">不限</a></li>
                    {foreach $page->getArea['行政区域'] as $k=>$v}
                        {if $k<8 || $page->getKeyword['admin_area']==$v}
                        ...
                        {/if}
                    {/foreach}
                    {if count($page->getArea['行政区域'])>8}
                    <li class="more">
                        <a href="javascript:;" class="more-cate">更多</a>
                        <ul class="clearfix">
                            {foreach $page->getArea['行政区域'] as $k=>$v}
                                {if $k>=8 && $page->getKeyword['admin_area']!=$v}
                                    <li><a href="{$_home}space/{$page->getUrls('admin_area',$v)}">{$v}</a></li>
                                {/if}
                            {/foreach}
                        </ul>
                    </li>
            ...
            {/if}
            {if count($page->getBusinessArea['所属商圈'])>0}
            <div class="search_select_section " {if !empty($page->getKeyword['admin_area'])}style="display:none"{/if}>
                <div class="search_select_section_left">
                    所属商圈
                </div>
                <ul id="search_area_type" class="search_select_section_right" style="{if !empty($page->getKeyword['business_area'])}height: auto;{/if}">
                    <li {if empty($page->getKeyword['business_area'])}class="search_select_active"{/if}><a  href="{$_home}space/{Web_Search_Item_List::getUrls('business_area','')}">不限</a></li>
                    {foreach $page->getBusinessArea['所属商圈'] as $k=>$v}
                        {if $k<8 ||  $page->getKeyword['business_area']==$v}
                        ...
                        {/if}
                    {/foreach}
                    {if count($page->getBusinessArea['所属商圈'])>8}
                    <li class="more">
                        <a href="javascript:;" class="more-cate">更多</a>
                        <ul class="clearfix">
                            {foreach $page->getBusinessArea['所属商圈'] as $k=>$v}
                                {if $k>=8 && $k<=30 && $page->getKeyword['business_area']!=$v}
                                    <li><a href="{$_home}space/{$page->getUrls('business_area',$v)}">{$v}</a></li>
                                {/if}
                            {/foreach}
                        </ul>
                    </li>
            ...
            <div class="search_select_section section_more_less">
                ...
                <ul class="search_select_section_right clearfix">
                    <li class="{if empty($page->getKeyword['people_range'])}search_select_active{/if}">
                        <a href="{$_home}space/{$page->getUrls('people_range','')}">不限</a>
                    </li>
                    {foreach $page->getPeople as $k=>$v}
                        <li class="{if $page->getKeyword['people_range']==$k}search_select_active{/if}">
                            <a href="{$_home}space/{$page->getUrls('people_range',$k)}">{$v}</a>
                        </li>
                    {/foreach}
                    <li class="cus-input">
                        <form action="{$_home}space" method="get">
                            <span>自定义</span>&nbsp;&nbsp;
                            {include  file="Web_Static::search/inc/inc.search_keyword.tpl"}
                            <input name="people_range[0]" type="text" value="{$page->customShow['people_range'][0]}"/>&nbsp;-&nbsp;<input
                                    type="text" name="people_range[1]" value="{$page->customShow['people_range'][1]}">&nbsp;人
                            <button type="submit" class="cus-price-ok none">确定</button>
                        </form>
                    </li>
                </ul>
            ...
            <div class="search_select_section section_more_less {if empty($page->getKeyword['industry_type'])}hide{/if}">
                ...
                <ul id="search_area_type" class="search_select_section_right">
                    <li class="{if empty($page->getKeyword['industry_type'])}search_select_active{/if}">
                        <a href="{$_home}space/{$page->getUrls('industry_type','')}">不限</a>
                    </li>
                    {foreach $page->getIndustryType as $k=>$v}
                        <li class="{if $page->getKeyword['industry_type']==$v}search_select_active{/if}">
                            <a href="{$_home}space/{$page->getUrls('industry_type',$v)}">{$v}</a>
                        </li>
                    {/foreach}
                    <div class="clear"></div>
                </ul>
            ...
            <div class="search_select_section section_more_less clearfix">
                <div class="search_select_section_left">
                    空间面积
                </div>
                <ul id="search_area_type" class="search_select_section_right clearfix">
                    <li class="{if empty($page->getKeyword['size_range'])}search_select_active{/if}">
                        <a href="{$_home}space/{$page->getUrls('size_range','')}">不限</a>
                    </li>
                    {foreach $page->getSize as $k=>$v}
                        <li class="{if $page->getKeyword['size_range']==$k}search_select_active{/if}">
                            <a href="{$_home}space/{$page->getUrls('size_range',$k)}">{$v}</a>
                        </li>
                    {/foreach}
                    <li class="cus-input">
                        <form action="{$_home}space" method="get">
                            <span>自定义</span>&nbsp;&nbsp;
                            {include  file="Web_Static::search/inc/inc.search_keyword.tpl"}
                            <input type="text" name="size_range[0]" value="{$page->customShow['size_range'][0]}"/>&nbsp;-&nbsp;
                            <input type="text" name="size_range[1]" value="{$page->customShow['size_range'][1]}">&nbsp;㎡
                            <button href="" class="cus-price-ok none">确定</button>
                        </form>
                    </li>
                </ul>
            </div>
            <div class="search_select_section section_more_less">
                ...
                <ul class="search_select_section_right clearfix">
                    <li class="{if empty($page->getKeyword['is_outside'])}search_select_active{/if}">
                        <a href="{$_home}space/{$page->getUrls('is_outside','')}">不限</a>
                    </li>
                    <li class="{if $page->getKeyword['is_outside']==5}search_select_active{/if}">
                        <a href="{$_home}space/{$page->getUrls('is_outside',5)}">开放</a>
                    </li>
                    <li class="{if $page->getKeyword['is_outside']==3}search_select_active{/if}">
                        <a href="{$_home}space/{$page->getUrls('is_outside',3)}">半开放</a>
                    </li>
                    <li class="{if $page->getKeyword['is_outside']==1}search_select_active{/if}">
                        <a href="{$_home}space/{$page->getUrls('is_outside',1)}">室内</a>
                    </li>
                </ul>
                <div class="clear"></div>
            </div>
...
<div class="more_less_wrap">
    ...
    <div class="search_select_outside none clearfix" id="search_selected">
        <div class="search_select clearfix">
            <div class="search_selected_title">搜索条件</div>
...
<div class="search-sort clearfix">
    <div class="fl search-sort-list-new">
        <a href="{$_home}space/{$page->getUrls('rank',0)}" class="{if empty($page->getKeyword['rank'])}active{/if}" style="font-size: 1em;">{if !empty($page->getItemSearchList['page']) && !empty($page->getKeyword['lat']) && !empty($page->getKeyword['lng']) && empty($page->getKeyword['admin_area']) && empty($page->getKeyword['business_area'])}距离{else}推荐{/if}</a>
        <span class="line" style="padding-right: 15px;padding-left: 15px;">|</span>
        <a class="sort sort-jg {if $page->getKeyword['rank']=='pup' || $page->getKeyword['rank']=='pdown'}active{/if}"
           href="javascript:;">价格<i class="icon-sort"></i></a>
        <span class="line" style="padding-right: 15px;padding-left: 15px;">|</span>
        <a class="sort sort-zs {if $page->getKeyword['rank']=='iup' || $page->getKeyword['rank']=='idown'}active{/if}"
           href="javascript:;">指数<i class="icon-sort"></i></a>
    </div>
...
{if count($page->getItemSearchList['rows'])>0}
    {foreach $page->getItemSearchList['rows'] as $k=>$v}
        <div class="search_list clearfix" data-lat="{$v['geo_lat']}" data-addr="{$v['addr']}" data-lng="{$v['geo_lng']}" data-name="{$v['baike_item_name']}" data-href="{$_home}site/{Func_Url::str_urlencode($v['baike_item_name'])}-百科"
             data-id="{$v['baike_id']}">
            <div class="search_list_title_outside clearfix">
                <div class="search_list_title">
                    ...
                    <p class="fr price">
                        {if !empty($v['min_price']) && $v['min_price']>100}￥{$v['min_price']}{if !empty($v['price_unit'])}{$v['price_unit']}{else}元/天
                        {/if}&nbsp;{if $v['item_count']>1 || $v['item_max_price']>$v['min_price']}起{/if}{else}面议{/if}</p>
                </div>
            </div>
            <div class="search_list_content clearfix">
                ...
                <div class="search_list_des clearfix">
                    ...
                    <div class="search_list_web">
                        ...
                        <span class="space-fit-numb"><span id="fit-item-{$v['baike_id']}">{$v['item_count']}
                                个适合空间/</span><span id="count-item-{$v['baike_id']}">共{$v['item_count']}个空间</span></span>
                    </div>
                    <!--浏览量、指数-结束-->
                </div>
                <!--右侧内容-结束-->
                    ...
                    <tr id="more-item-{$v['baike_id']}" class="hide">
                        <td colspan="7">
                            <a class="lookmore"
                               ...
                               target="_blank">查看更多空间({$v['item_count']})</a>
                        </td>
                    </tr>
                </table>
            </div>
    ...
    {/foreach}
{else}
    <div class="search-no" style="margin-bottom: 30px;">
        <div class="search-no-fig">找到相关空间0个</div>
        <div class="search-no-title">
            {if !empty(array_filter($page->showScreenValue))}
                抱歉，没有找到{if !empty($page->getKeyword['keyword2'])}“<span>{$page->getKeyword['keyword2']}</span>”
                {if !empty($page->getItemSearchList['page']) && empty($page->getKeyword['admin_area']) && empty($page->getKeyword['business_area'])}
                    附近,
                {else}
                    相关,
                {/if}{/if}符合“<span style="color:red;">
                {foreach array_filter($page->showScreenValue) as $k=>$v}
                    {$v},
                {/foreach}
                </span>
                ”的空间
            {else}
                抱歉，没有找到{if !empty($page->getKeyword['keyword2'])}“<span>{$page->getKeyword['keyword2']}</span>”{/if}
                {if !empty($page->getItemSearchList['page']) && empty($page->getKeyword['admin_area']) && empty($page->getKeyword['business_area'])}
                    附近的空间
                {else}
                    相关的空间！
...
{/if}
<!--------------------数据显示结束------------------------------->
{if count($page->getSearchKeywordList)>0}
    <div class="related_search">
        <div class="related_title">相关搜索</div>
        <ul class="related_content">
            {foreach $page->getSearchKeywordList  as $k=>$v}
                {if $v['search_title']!=$page->getKeyword['keyword2']}
                <li><a href="{$_home}space?keyword2={Func_Url::str_urlencode($v['search_title'])}"> {$v['search_title']}</a>
                </li>
                {/if}
            {/foreach}
            <div class="clear"></div>
...
{/if}
<!--------------------分页--开始----------------------------->
{if count($page->getItemSearchList['rows'])>0 && !empty($page->getItemSearchList['page'])}
<div class="pagers">
...
<div class="search_content_right_no clearfix">
    {*<ul class="search-no-right classify_rt_img">*}
        ...
        {*<li>*}
            {*<div class="search-no-right-weixin">*}
                {*<img src="css/search.v4.0/img/search-no-icon3.jpg">*}
            {*</div>*}
    ...
    {*</div>*}
        <div class="map-section">
            <a href="{$_home}map2" target="_blank" class="to-map">
                <span>查看场地地图</span>
            </a>
            <div id="allmap4"></div>
            <div class="sao-code clearfix">
                <img src="img/qrcode_info.png" alt="" class="fl">
                <div class="phone-icon fl"></div>
                <div class="icon-text fl">
                    <span class="p1">举起手机,扫一扫关注</span>
                    <span class="p2">云·space微信公众号<br/>随时随地获取方案</span>
                </div>
            </div>
        </div>
...
<!--查询档期和价格弹窗-结束-->
{*{if !empty($page->getItemSearchList)}*}
...
{static "search.v4.0/search3.js"}
{static "search.v4.0/highlight_item.js"}
{*{$page->getKeyword['keyword2']|var_dump}*}
<script>
    function shaixuan() {
        var selectBut = '';
        {if !empty($page->getKeyword['keyword2'])}
        selectBut +='<li style="color:#ababab!important;"><span class="keyword">{if !empty($page->getItemSearchList['page']) && empty($page->getKeyword['admin_area']) && empty($page->getKeyword['business_area'])}位置{else}关键词{/if}：{$page->getKeyword['keyword2']}</span></li>';
        {/if}
        {foreach $page->getScreenValue as $k=>$v}
        {if !empty($v)}
        selectBut += ' <li><span>{$v}</span>' +
        '<a href="{$_home}space/{$page->getUrls($k,'')}"></a></li>';
        {/if}
        {/foreach}
//        console.log(selectBut);
        if (selectBut) {
            selectBut += '<li><a href="{$_home}space/{array_search($page->getKeyword['city'],Web_Global::$city_name)}" class="search_selected_deleted"> 全部删除</a></li>';
...
//        $('.screen').html(selectBut);
    ...
    var people_range = "{$page->getScreenValue['people_range']}";
    var industry_type = "{$page->getScreenValue['industry_type']}";
    if (size_range != '' || is_outside != '' || industry_type != '' || people_range!='') {
    ...
    // js请求子空间使用
    window.city = "{$page->getKeyword['city']}";
    window.search_type = "{$page->getKeyword['search_type']}";
    window.category = "{$page->getKeyword['category']}";
    window.activeType = "{$page->getKeyword['active_type']}";
    window.size_range = "{$page->getKeyword['size_range']}";
    window.people_range = "{$page->getKeyword['people_range']}";
    window.is_outside = "{$page->getKeyword['is_outside']}";
    window.keyword = "{$page->getKeyword['keyword2']}";
    window.lng = "{$page->getKeyword['lng']}";
    window.lat = "{$page->getKeyword['lat']}";
    {if empty($page->getKeyword['admin_area']) && empty($page->getKeyword['business_area']) && !empty($page->getItemSearchList['page'])}
    window.is_distance = "1";
    {else}
    window.is_distance = "";
    {/if}
    // 价格、指数排序
    ...
    $('.search-sort-list-new .sort-jg').on('click', function () {
        ...
        if (tsort.hasClass('down')) {
            ...
            $('.sort-jg').attr('href', "{$_home}space/{$page->getUrls('rank','pup')}");
            tsort.addClass('up');
        } else {
            $('.sort-jg').attr('href', "{$_home}space/{$page->getUrls('rank','pdown')}");
            tsort.addClass('down');
        }
    });
    <!--指数-->
    $('.search-sort-list-new .sort-zs').on('click', function () {
        ...
        if (tsort_zs.hasClass('up')) {
            ...
            $('.sort-zs').attr('href', "{$_home}space/{$page->getUrls('rank','idown')}");
            tsort_zs.addClass('down');
        } else {
            $('.sort-zs').attr('href', "{$_home}space/{$page->getUrls('rank','iup')}");
