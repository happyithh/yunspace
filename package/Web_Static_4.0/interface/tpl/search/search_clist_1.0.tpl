...
{include file="Web_Static::inc/header_1.tpl"}
    ...
    <div class="search_content">
        {if $page->getCompanyList['rows']}
            <p class="search_word_top">云·SPACE&nbsp;为您找到相关结果约{$page->getCompanyList['page']['dt']}条</p>
        {/if}
        <div class="search_content_left mt25">
            {if $page->getCompanyList['rows']}
          {foreach $page->getCompanyList['rows'] as $k=>$v}
              <div class="search_list">
                  <div class="search_list_title_outside">
                      <div class="search_list_title">
                          <a href="{$_home}vendor_info/{$v['id']}.html" target="_blank"> {$v['vendor_name']}</a>
                      </div>
                      <div class="search_site_icon mt35">
                          <a href="javascript:;" class="j_search_date" data-city="{$v['attr']['城市']}" data-id="{$v['id']}" data-name="{$v['vendor_name']}"> <div class="search_wt"></div></a>
                      </div>
                      <div class="clear"></div>
                  </div>
                  <div class="search_list_content">
                      ...
                      <div class="search_list_img">
                          <a href="{$_home}vendor_info/{$v['id']}.html" target="_blank">
                      ...
                      <div class="search_list_des">
                          <div class="{if $v['logo']}search_list_word{else}search_list_word1{/if}">
                             简介： {mb_substr($v['des'],0,100,'utf-8')}…<br/>
                          </div>
                          <div class="search_list_web">
                              <span class="browse">服务行业 :  {$v['attr_index']['服务行业']}</span>
                              {*<span class="browse">浏览量：{$v['counter_view']} </span>*}
                              {*<span class="indexs">指数：{$v['counter_praise']} </span>*}
                          </div>
                      </div>
                      <div class="clear"></div>
                  </div>
            ...
            {else}
                <div class="search-no">
                    <div class="search-no-fig">找到相关公司0个</div>
                    <div class="search-no-title">抱歉，没有找到与“<span>{$_REQUEST['keyword']}</span>”相关的公司内容！</div>
            ...
            <div class="related_search">
                ...
                <ul class="related_content">
                    {foreach $page->getSearchList  as $k=>$v}
                        <li><a href="{$_home}company-keyword/{urlencode($v['search_title'])}"> {$v['search_title']}</a></li>
                    {/foreach}
                    <div class="clear"></div>
                </ul>
            </div>
            {include file="Web_Static::inc/paging_com.tpl"}
        </div>
...
<div class="search-box none" id="search-code">
    ...
    <input type="hidden" name="from_type" class="from_type" value="3" >
