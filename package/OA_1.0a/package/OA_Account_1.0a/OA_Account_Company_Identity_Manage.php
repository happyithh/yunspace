 ...
 * Date: 15/8/6
 */
class OA_Account_Company_Identity_Manage extends Base
{
    static function handle($accountType = 1)
    {
        ...
        switch ($action) {
            case 'list':
                ...
                break;
            case 'details':
                self:: handleCompanyIdentityDetails();
                break;
            case 'reject':
                $_REQUEST['status'] = -1;
                self:: handleCompanyIdentityChangeStatus();
                break;
            case 'pass':
                $_REQUEST['status'] = 1;
                self:: handleCompanyIdentityChangeStatus();
...
//                break;
    ...
    static function handleList()
    {
        ...
        $rs = Api_Account_Identity_Company_List::getList($_REQUEST);
        ...
        Tpl_Smarty::display('OA_Static::account_identity/account_identity_company_list.tpl');
    }
    ...
    static function handleCompanyIdentityDetails()
    {

        $res = Api_Account_Identity_Get_Company_Info_By_Id::getInfo($_REQUEST);
        if($res['status']==0){
...
//        Tpl_Smarty::display('OA', 'account_identity_info.tpl');
        Tpl_Smarty::display('OA_Static::account_identity/account_identity_company_info.tpl');
    }
    ...
    static function handleCompanyIdentityChangeStatus()
    {
        $rs =Api_Account_Identity_Change_Company_Status_By_Id::changeStatus($_REQUEST);
        if ($rs['status']==0) {
